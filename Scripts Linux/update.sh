...
if [ $? != 1 ]; then # Si la commande passe = 1 sinon = 0 
	...
	sudo echo "Update $(hostname) $(hostname -I) : Mise à jour de la liste des paquets faite le $(date +"%d/%m/%y %k:%M")" >> /var/log/logsscripts.log
	...
	sudo echo "Upgrade $(hostname) $(hostname -I) : Mise à jour des paquets faite le $(date +"%d/%m/%y %k:%M")" >> /var/log/logsscripts.log
else
	# Si les commande ne passe pas écrit dans le fichier de log la date de l'erreur
	sudo echo "Erreur sur le serveur $(hostname) $(hostname -I) de la mise à jour le $(date +"%d/%m/%y %k:%M")" >> /var/log/logsscripts.log
